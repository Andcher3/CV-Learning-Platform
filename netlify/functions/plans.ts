...
const getPretestFilePath = (unitId: number) => {
  const folder = path.join(DATA_DIR, `admin/unit_plan/unit${unitId}`);
...
const resolvePretestFilePath = (unitId: number) => {
  ...
  const fallback = path.join(localDataRoot, `admin/unit_plan/unit${unitId}`, `unit${unitId}plantest.md`);
  ...
  if (req.method === 'POST' && url.pathname === '/api/plans/generate') {
    ...
    try {
      ...
      if (!existing && !pretestQuestion) {
        return new Response(JSON.stringify({ error: '首次生成学习计划前，未找到该单元的预设测评题文件。' }), { status: 400 });
      }
