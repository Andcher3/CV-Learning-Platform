...
try {
  db.exec("ALTER TABLE notes ADD COLUMN file_names TEXT");
} catch (e) {
  // Column might already exist
}
try {
...
const parseJsonSafe = <T>(raw: any, fallback: T): T => {
...
const normalizeChoice = (value: any) => String(value || '').trim().toUpperCase();

const fixTransformerEasy11AnswerKey = () => {
  const targetRows = db.prepare(`
    SELECT id, answer_key, quiz_payload, student_answers, total_questions, submitted_at
    FROM quiz_assignments
    WHERE unit_id = 5 AND answer_key LIKE ?
  `).all('%"id":"easy-11"%') as any[];

  if (!targetRows.length) return;

  const updatePending = db.prepare('UPDATE quiz_assignments SET answer_key = ? WHERE id = ?');
  const updateSubmitted = db.prepare(
    'UPDATE quiz_assignments SET answer_key = ?, grading_detail = ?, correct_count = ?, score = ? WHERE id = ?'
  );

  let fixedCount = 0;
  let regradedCount = 0;

  const applyFix = db.transaction((rows: any[]) => {
    for (const row of rows) {
      const answerRows = parseJsonSafe<any[]>(row.answer_key, []);
      if (!Array.isArray(answerRows) || answerRows.length === 0) continue;

      let changed = false;
      for (const item of answerRows) {
        if (String(item?.id || '').trim() !== 'easy-11') continue;
        if (normalizeChoice(item?.correctAnswer) !== 'B') {
      ...
      fixedCount += 1;
      const answerKeyJson = JSON.stringify(answerRows);

      if (!row.submitted_at) {
        updatePending.run(answerKeyJson, row.id);
        continue;
      }

      const questions = parseJsonSafe<any[]>(row.quiz_payload, []);
      const submittedAnswers = parseJsonSafe<Record<string, string>>(row.student_answers, {});
      ...
      for (const item of answerRows) {
        ...
        if (!id) continue;
        answerMap.set(id, {
          correctAnswer: normalizeChoice(item?.correctAnswer),
      ...
      const gradingDetail = questions.map((question: any) => {
        ...
        const userAnswer = normalizeChoice(submittedAnswers?.[id]);
        const answer = answerMap.get(id);
        const correctAnswer = normalizeChoice(answer?.correctAnswer);
      ...
      const totalQuestions = Number(row.total_questions || questions.length || 0);
      ...
      updateSubmitted.run(answerKeyJson, JSON.stringify(gradingDetail), correctCount, score, row.id);
      regradedCount += 1;
    }
  });
  ...
  applyFix(targetRows);
  if (fixedCount > 0) {
    console.log(`[db] fixed unit5 easy-11 answer key for ${fixedCount} assignments; regraded ${regradedCount} submitted assignments.`);
  }
};

fixTransformerEasy11AnswerKey();
