const PLAN_SUBMISSION_TYPE_RULES = `
[提交物类型约束（必须严格执行）]
当你在学习计划中要求学生提交“文件类成果”时，必须只要求 AI 可识别的文件类型。

允许要求的提交物示例（优先从下列类型中选择，并优先选择文本/代码/文档与图片）：
- 文本与代码：.md, .txt, .pdf, .ipynb, .py, .js, .ts, .tsx, .json, .yaml, .yml, .csv, .html, .css, .java, .cpp, .c, .go, .sh
- 文档：.doc, .docx, .ppt, .pptx
- 图片与视频：.png, .jpg, .jpeg, .webp, .gif, .mp4, .mov, .avi, .mkv
- 音频：.mp3, .wav, .m4a

禁止要求的提交物（严禁出现在计划中）：
- 压缩包与归档：.zip, .rar, .7z, .tar, .gz
- 项目链接或代码仓库链接（如 GitHub/Gitee URL）
- 网盘分享链接、在线文档链接、需要跳转外部网站才能查看的材料

若确需提交“项目结果”，请改为要求学生提交可识别文件本体（如 README.md + 关键源码文件 + 运行截图），而不是链接或压缩包。
`;
...
${PLAN_SUBMISSION_TYPE_RULES}
`,

  planRetry: (currentPrompt: string) => `${currentPrompt}

再次强调：若计划中包含“提交物要求”，只能要求 AI 可识别文件类型；禁止 zip/rar/7z 和任何项目链接、网盘链接。
...
关键约束（必须严格执行）：
1) 你必须基于“先前的学习计划”做增删改，不允许从零生成一份全新计划。
2) 优先保留先前计划中的任务骨架与顺序；仅对必要任务进行删除、拆分、合并、改写或重排。
3) 若需新增任务，只能为弥补已识别缺口而少量新增，并与原目标直接相关。
4) 不得输出与先前计划风格完全不同的模板化新大纲（例如完全重建分阶段总纲）。
5) 若学生笔记证据不足以支持大改，请采用“最小改动原则”。

提交物约束（必须执行）：
- 如果你在调整后的计划中要求提交文件，必须只要求 AI 可识别文件类型（见下方规则），禁止要求压缩包或任何项目链接。
...
3) 明确“剩余时间内”的日程与任务顺序；
4) 输出结果必须体现为“在原计划基础上的调整版”，而不是全新重写版；
5) 直接返回计划内容，不要输出思考过程。`,
...
证据完整性规则（必须执行）：
- 若输入材料中出现“中间省略/文末片段/截断说明”等字样，代表系统为控制长度做了抽样，不可据此直接判定“未完成”。
- 当你无法确认某任务是否完成时，应标记为“证据不足，需补充材料复核”，并采用保守扣分（避免重扣）。
- 仅当有明确反证（如明确写明未完成）时，才可判定该任务未完成。
